...
class KitSexy extends PluginBase implements Listener {
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $args){
		if(strtolower($cmd->getName()) === "kit"){
			...
			switch(strtolower($args[0])){
				case "list":
					...
					if ($normalKits !== 'Normal Kits: ')
					{
						$sender -> sendMessage("< GCS - NETWORK > " . $normalKits);
					}
					if ($donatorKits !== 'Donator Kits: ')
					{
						$sender -> sendMessage("< GCS - NETWORK > " . $donatorKits);
					}
						return true;
					break;
				case "":
					...
					{
						$sender->sendMessage('< GCS - NETWORK > You need to die before you can pick a new kit!');
						return true;
					}
					else
					{
						$sender->sendMessage("< GCS - NETWORK > Usage: /kit <kit name> or /kit list");
						return true;
					}
					break;
				default:
					if (!$sender instanceof Player)
					{
						$sender->sendMessage("< GCS - NETWORK > Consoles don't need kits!");
						return true;
					}
					$username = $sender->getName();
					if (in_array($username, $this -> players))
			...
			if(strtolower($cmd->getName()) == "donator"){
				...
				switch(strtolower($args[0])){
					...
					$sender->sendMessage("< GCS - NETWORK > Usage: /donator <add|rmv> <exact username>");
					return true;
				}
			}
		}
		...
		public function onPlayerDeath(PlayerDeathEvent $event){
			$event->getEntity()->sendMessage("< GCS - NETWORK > You died and may now pick a new kit!");
